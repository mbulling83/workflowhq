...
- Prefers benefit-first homepage copy and pushes to remove implementation-detail language (for example, stack mentions like React/Next.js).
...
- Pricing visibility is controlled by the Vercel flag key `show-pricing-section`, with `app/flags.ts` fallback defaulting to hidden.
